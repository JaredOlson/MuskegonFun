...
Thanks to the original author, Alexandre:
#  Author: Alexandre Tedeschi (d)
#  E-Mail: alexandrebr at gmail dot com
#  Londrina - PR / Brazil

Via personal communication on 10-October-2011, Alexandre asserted that this code is licensed 
under the BUGROFF license: http://tunes.org/legalese/bugroff.html
...
class dUnzip2{
	...
	Function unzip($compressedFileName, $targetFileName=false, $applyChmod=0777){
		...
		// ---- TODO: Fix this to use avoid memory cap problems. ----
	...
	Function unzipAll($targetDir=false, $baseDir="", $maintainStructure=true, $applyChmod=0777){
		...
		if(sizeof($lista)) foreach($lista as $fileName=>$trash){
			...
			// Check for no-go files [NR mod 25-Aug-11, 7-May-12]
			if (!preg_match('/\.(html|htm|jpg|jpeg|png|gif|mp4|flv|f4v|mpg4|m4v|mp3)$/i', $fileName))
				continue;
